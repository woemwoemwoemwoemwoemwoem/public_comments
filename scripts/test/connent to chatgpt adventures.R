...
t1 <- read.table("../text files/tmp01.txt", header = FALSE, sep = "\t", stringsAsFactors = FALSE)
...
# links <- df[-c(1,2,nrow(df)),]
net <- as.network(df[-c(1,2,nrow(df)),],ignore.eval = "FALSE")
...
tmpout <- forceNetwork(Nodes = nodes, Links = links
                       , charge=-10
                       , NodeID = "concept"
                       , Group = "group"
                       , Nodesize = "deg"
...
); saveNetwork(tmpout,"test net elbroch nyt.html")
